...
package hamr.SW_Impl_Instance_FLT_ARes_CASE_Filter_ARes
...
import hamr._
import hamr.SW_AUGUST.CASE_Filter_ARes_thr_Impl_seL4Nix

object CASE_Filter_ARes extends App {

  val CASE_Filter_AResBridge : hamr.SW_AUGUST.CASE_Filter_ARes_thr_Impl_Bridge = {
  ...
  val entryPoints: Bridge.EntryPoints = CASE_Filter_AResBridge.entryPoints
  ...
  def getValue(portId: Art.PortId): Option[DataContent] = {
    if(portId == filter_in_id) {
      ...
      halt(s"Unexpected: CASE_Filter_ARes.getValue called with: ${portId}")
    }
  }
  ...
  def receiveInput(eventPortIds: ISZ[Art.PortId], dataPortIds: ISZ[Art.PortId]): Unit = {
    ...
    filter_in_port = CASE_Filter_ARes_thr_Impl_seL4Nix.filter_in_Receive()
  }
  ...
  def initialiseArchitecture(): Unit = {
    val ad = ArchitectureDescription(
      components = MSZ (CASE_Filter_AResBridge),
      connections = ISZ ()
    )
    Art.run(ad)
  }
