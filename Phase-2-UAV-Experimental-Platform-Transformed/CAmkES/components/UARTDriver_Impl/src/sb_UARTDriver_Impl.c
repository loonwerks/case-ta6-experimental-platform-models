...
Unit hamr_Drivers_UARTDriver_Impl_seL4Nix_AirVehicleState_WPM_Send(
  STACK_FRAME
  art_DataContent d) {
  DeclNewStackFrame(caller, "sb_UARTDriver_Impl.c", "", "hamr_Drivers_UARTDriver_Impl_seL4Nix_AirVehicleState_WPM_Send", 0);

  sb_AirVehicleState_WPM_enqueue(d);
}

// send AirVehicleState_UXAS: Out EventDataPort CMASI__AirVehicleState_i
Unit hamr_Drivers_UARTDriver_Impl_seL4Nix_AirVehicleState_UXAS_Send(
  STACK_FRAME
  art_DataContent d) {
  DeclNewStackFrame(caller, "sb_UARTDriver_Impl.c", "", "hamr_Drivers_UARTDriver_Impl_seL4Nix_AirVehicleState_UXAS_Send", 0);

  sb_AirVehicleState_UXAS_enqueue(d);
}

// FIXME: dummy implementation for unconnected outgoing port
Unit hamr_Drivers_UARTDriver_Impl_seL4Nix_send_data_Send(
  STACK_FRAME
  art_DataContent d) {
  DeclNewStackFrame(caller, "sb_UARTDriver_Impl.c", "", "hamr_Drivers_UARTDriver_Impl_seL4Nix_send_data_Send", 0);
...
Unit hamr_Drivers_UARTDriver_Impl_seL4Nix_MissionCommand_Receive(
  STACK_FRAME
  Option_8E9F45 result) {
  DeclNewStackFrame(caller, "sb_UARTDriver_Impl.c", "", "hamr_Drivers_UARTDriver_Impl_seL4Nix_MissionCommand_Receive", 0);
...
// FIXME: dummy implementation for unconnected incoming port
Unit hamr_Drivers_UARTDriver_Impl_seL4Nix_recv_data_Receive(
  STACK_FRAME
  Option_8E9F45 result) {
  DeclNewStackFrame(caller, "sb_UARTDriver_Impl.c", "", "hamr_Drivers_UARTDriver_Impl_seL4Nix_recv_data_Receive", 0);
