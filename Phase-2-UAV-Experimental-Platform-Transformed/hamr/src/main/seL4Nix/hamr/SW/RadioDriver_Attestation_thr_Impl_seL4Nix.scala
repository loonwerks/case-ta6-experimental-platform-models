...
@ext object RadioDriver_Attestation_thr_Impl_seL4Nix {
  ...
  // returns T if seL4's am_request port is empty, F otherwise 
  def am_request_IsEmpty(): B = $

  // returns result of dequeuing seL4's am_request port 
  def am_request_Receive(): Option[DataContent] = $

  // send payload 'd' to components connected to seL4's am_response port
  def am_response_Send(d: DataContent): Unit = $
}
